...
"""
Filesystem router — thin HTTP layer.
Delegates CRUD to FilesystemService, caching to FileCache.
Supports ETag headers and optimistic locking with If-Match.
"""
...
from dependencies import get_sandbox, get_fs_agent
...
logger.info(
    f"File cache initialized: max_size={settings.FILE_CACHE_MAX_SIZE}, "
    f"ttl={settings.FILE_CACHE_TTL_SECONDS}s"
)
...
async def get_file_tree(
    ...
    fs_agent=Depends(get_fs_agent),
):
    sandbox, sid = sb_and_id
    safe_path = _validate_user_path(user_id, path) if path else ""
    try:
        tree = await fs_agent.get_tree(safe_path, sandbox, max_depth)
...
async def list_files(
    ...
    safe_path = _validate_user_path(user_id, path) if path else ""
    try:
        files = await fs_agent.list_files(safe_path, sandbox)
        return {
...
async def read_file_content(
    ...
    try:
        file_info = await fs_agent.get_file_info(safe_path, sandbox)
        ...
        result = await fs_agent.read_file(safe_path, sandbox)
...
async def write_file_content(
    ...
    try:
        ...
        success = await fs_agent.write_file(safe_path, request.content, sandbox)
        if not success:
...
async def create_folder(
    ...
    safe_path = _validate_user_path(user_id, request.path)
    try:
        success = await fs_agent.create_folder(safe_path, sandbox, mode=request.mode)
        if not success:
...
async def create_file(
    ...
    safe_path = _validate_user_path(user_id, request.path)
    try:
        success = await fs_agent.create_file(safe_path, sandbox, request.content)
        if not success:
...
async def delete_path(
    ...
    safe_path = _validate_user_path(user_id, path)
    try:
        success = await fs_agent.delete_path(safe_path, sandbox, recursive)
        if not success:
...
async def rename_path(
    ...
    Examples:
    - Rename file: {"source": "old.py", "destination": "new.py"}
    - Rename folder: {"source": "old_folder", "destination": "new_folder"}
    - Move file: {"source": "file.py", "destination": "folder/file.py"}
    - Move folder: {"source": "src", "destination": "backup/src"}
    """
    sandbox, sid = sb_and_id
    safe_source = _validate_user_path(user_id, request.source)
    safe_dest = _validate_user_path(user_id, request.destination)
    try:
        success = await fs_agent.move_files(safe_source, safe_dest, sandbox)
        if not success:
...
async def find_in_files(
    ...
    try:
        matches = await fs_agent.find_in_files(safe_path, request.pattern, sandbox)
        return {
            "success": True,
            "sandbox_id": sid,
            "user_id": user_id,
...
async def search_files(
    ...
    safe_path = _validate_user_path(user_id, request.path) if request.path else ""
    try:
        matches = await fs_agent.search_files(request.pattern, sandbox, safe_path)
        return {
            "success": True, "sandbox_id": sid, "user_id": user_id,
...
async def replace_in_files(
    ...
    try:
        results = await fs_agent.replace_in_files(
...
async def set_file_permissions(
    ...
    safe_path = _validate_user_path(user_id, request.path)
    try:
        success = await fs_agent.set_file_permissions(
            safe_path,
            sandbox,
            mode=request.mode,
            owner=request.owner,
...
async def cleanup_workspace(
    ...
    sandbox, sid = sb_and_id
    try:
        success = await fs_agent.cleanup(sandbox)
        if not success:
