...
class FilesystemService:
    ...
    async def get_tree(self, path: str, sandbox, max_depth: int = 3) -> dict[str, Any]:
        """Build tree from SDK list_files + get_file_info only (no custom list)."""
        try:
            sdk_path = _workspace_path(path)
            rel_base = path or ""
    ...
    async def delete_path(self, path: str, sandbox, recursive: bool = False) -> bool:
        try:
            sdk_path = _workspace_path(path)
            if not sdk_path.startswith("workspace"):
                logger.error(f"Security: delete outside workspace: {sdk_path}")
                return False
            sandbox.fs.delete_file(sdk_path, recursive=recursive)
