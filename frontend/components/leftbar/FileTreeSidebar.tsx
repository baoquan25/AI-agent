...
type FileTreeSidebarProps = {
  width: number;
  leftBarTab: 'files' | 'search';
  setLeftBarTab: (tab: 'files' | 'search') => void;
  openSearchTab: () => void;
...
export function FileTreeSidebar(props: FileTreeSidebarProps) {
  ...
  return (
    <div style={{ width, minWidth: width > 0 ? 80 : 0, flexShrink: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div className="section" id="leftBar">
        <div className="leftbar-toolbar">
          <button
            type="button"
            className={`leftbar-tab ${leftBarTab === 'files' ? 'active' : ''}`}
          ...
          <button
            type="button"
            className={`leftbar-tab ${leftBarTab === 'search' ? 'active' : ''}`}
            onClick={() => {
              openSearchTab();
            }}
            title="Search (Ctrl+P)"
